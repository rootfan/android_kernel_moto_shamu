...
#define SUBSYS(_x)								 \
	struct static_key _x ## _cgrp_subsys_enabled_key = STATIC_KEY_INIT_TRUE; \
	struct static_key _x ## _cgrp_subsys_on_dfl_key = STATIC_KEY_INIT_TRUE;  \
...
static struct static_key *cgroup_subsys_enabled_key[] = {
#include <linux/cgroup_subsys.h>
};
#undef SUBSYS

#define SUBSYS(_x) [_x ## _cgrp_id] = &_x ## _cgrp_subsys_on_dfl_key,
static struct static_key *cgroup_subsys_on_dfl_key[] = {
#include <linux/cgroup_subsys.h>
};
#undef SUBSYS
...
static int rebind_subsystems(struct cgroup_root *dst_root, u16 ss_mask)
{
	...
	do_each_subsys_mask(ss, ssid, ss_mask) {
		...
		if (dst_root == &cgrp_dfl_root) {
			static_key_enable(cgroup_subsys_on_dfl_key[ssid]);
		} else {
			dcgrp->subtree_control |= 1 << ssid;
			static_key_disable(cgroup_subsys_on_dfl_key[ssid]);
		}
...
int __init cgroup_init(void)
{
	...
	for_each_subsys(ss, ssid) {
		...
		if (cgroup_disable_mask & (1 << ssid)) {
			static_key_disable(cgroup_subsys_enabled_key[ssid]);
